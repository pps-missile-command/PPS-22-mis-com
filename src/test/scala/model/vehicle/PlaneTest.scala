...
class PlaneTest extends AnyFunSpec:
    ...
    describe("A plane") {
        ...
        it("should move along right axis") {
            var vehicle = Plane(planeTypes.Left_To_Right, 0)
            vehicle = vehicle.timeElapsed(1)
            vehicle = vehicle.move()
            assert(vehicle.position === Point2D(5, 0))

            val tp = 2
            var vehicle2 = Plane(planeTypes.Left_To_Right, 0)
        ...
        it("should be with the correct direction type") {
            var vehicle = Plane(planeTypes.Left_To_Right, 0)
            var vehicle2 = Plane(planeTypes.Right_To_Left, 0)
            assert(vehicle.planeDirection === planeTypes.Left_To_Right)
            assert(vehicle2.planeDirection === planeTypes.Right_To_Left)
        }
    }
